  ...
  <NodeViewWrapper>
    <div class="relative bg-gray-200 h-max rounded-md">
      <img
        @click="opened = true"
        loading="lazy"
        @error="$refs[`data_img_error`].style.display = 'inline'"
        @load="$refs[`data_img_load`].style.display = 'none'"
        class="
          w-full
      ...
      >
        <div class="w-full h-full flex items-center justify-center">
          ...
          >
            ...
            <p class="select-none text-sm">{{ alt || "Loading..." }}</p>
          </div>
        </div>
      </div>
      <!-- error -->
    ...
    <Modal v-model="opened" dim closeOnEsc closeOnClickOutside dark>
      <ImageModal :src="src" :alt="alt || 'Loading...'" />
    </Modal>
  </NodeViewWrapper>
</template>
...
export default {
  ...
  setup(prop) {
    ...
    const src = computed(() => prop.node.attrs.src);
    const alt = computed(() => prop.node.attrs.alt)
